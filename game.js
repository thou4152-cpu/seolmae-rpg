...
function effectiveMaxHp(){const b=bossSetBonuses();return Math.round((Number(G.maxHp)||0)*(1+b.hpMp))}
function effectiveMaxMp(){const b=bossSetBonuses();return Math.round((Number(G.maxMp)||0)*(1+b.hpMp))}
...
function status(){
 ...
 return `<div class="panel mobileBattleStatus"><div class="mbsTop"><b>${JOBS[G.job].icon} ${JOBS[G.job].name} Lv.${G.level}</b><span>${goldText()} · 🔑${G.marks}/10</span></div>
 <div class="mbsBars"><div><b>HP ${Math.ceil(G.hp)}/${Math.ceil(effectiveMaxHp())}</b><div class="bar"><div class="fill" style="width:${100*G.hp/effectiveMaxHp()}%"></div></div></div>
 <div><b>MP ${Math.ceil(G.mp)}/${Math.ceil(effectiveMaxMp())}</b><div class="bar mp"><div class="fill" style="width:${100*G.mp/effectiveMaxMp()}%"></div></div></div></div></div>`;
}
...
let mobileBattleSpeed=1;
function toggleBattleSpeed(){mobileBattleSpeed=mobileBattleSpeed===1?2:1;renderBattle()}
function battleDelay(ms){return Math.max(70,Math.round(ms/mobileBattleSpeed))}
...
function renderBattle(){
 ...
 <img src="${enemyBattleImage(e)}"><div class="targetMark">${battle.selectedTarget===i?"🎯":""}</div><div class="enemyName">${e.name}</div>
 ...
 screen(battleStatus()+`<div class="panel"><div class="title">${battle.isBoss?"👑 BOSS BATTLE":"🌿 고블린 초원 · Stage "+G.stage}</div>
 ...
 <div class="battleUtility"><button class="stageBtn speedBtn" onclick="toggleBattleSpeed()">⏩ ×${mobileBattleSpeed}</button><span>적을 터치해 타겟 선택</span></div><div class="skillDock">
 ...
 </div><div class="mobileQuickBar">
 <button onclick="autoAttack()">⚔️<small>공격</small></button>
 <button onclick="quickSkill(0)">✨<small>${STARTER_SKILLS[G.job][0].name}</small></button>
 <button onclick="usePotion()">🧪<small>HP ${G.potions}</small></button>
 <button onclick="useEther()">🔷<small>MP ${G.ethers||0}</small></button>
...
function rewardCompareHtml(item){
 if(!item)return "";
 const eq=G.equipment?.[item.slot];
 if(!eq)return `<div class="rewardCompare good">현재 이 부위는 비어 있습니다.</div>`;
 const diff=item.stat===eq.stat?Number(item.value||0)-Number(eq.value||0):null;
 return `<div class="rewardCompare"><b>현재 장비와 비교</b><br>[${eq.grade}] ${eq.name} · ${itemStatText(eq)}<br>→ [${item.grade}] ${item.name} · ${itemStatText(item)}
 ${diff!==null?`<div class="${diff>0?"good":diff<0?"bad":"sub"}">${diff>0?"+":""}${item.stat==="crit"?Math.round(diff*100)+"%":diff}</div>`:""}</div>`;
}
function equipRewardItem(){
 const it=G.inventory[G.inventory.length-1];if(!it)return;
 const old=G.equipment[it.slot];if(old)G[old.stat]-=old.value;
 G.equipment[it.slot]=it;G[it.stat]+=it.value;clampResources();save();toast(`${it.name} 바로 장착!`);renderCharacterMenu("dungeon","equip");
}
function renderRewardScreen(xp,gold,item,markDrop,isBoss,bossExclusive=false){
 ...
 ${rewardCompareHtml(item)}
 <div class="rewardActions"><button class="stageBtn" onclick="equipRewardItem()">⚔️ 바로 장착</button><button class="stageBtn" onclick="toast('인벤토리에 보관했습니다.')">🎒 보관</button></div>
